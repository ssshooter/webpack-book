...
W>您应该严格使用WDS进行开发。如果要托管应用程序，请考虑其他标准解决方案，例如Apache或Nginx。
...
完成此更改后，您可以通过环境参数配置服务器主机和端口选项（例如：`PORT = 3000 npm start`）。
...
有时，WDS提供的文件观看设置将无法在您的系统上运行。在旧版本的Windows，Ubuntu，Vagrant和Docker上可能会出现问题。启用轮询是一个很好的选择：
