...
**服务器端呈现**（SSR）是一种允许您使用HTML，JavaScript，CSS甚至应用程序状态提供初始有效负载的技术。即使没有启用JavaScript，您也可以提供完全呈现的HTML页面。除了提供潜在的性能优势外，这还有助于搜索引擎优化（SEO）。
...
为了演示SSR，您可以使用webpack编译客户端构建，然后由服务器接收，该服务器遵循原则使用React呈现它。这样做足以了解它是如何工作的以及问题的开始。
...
T> SSR不是SEO问题的唯一解决方案。 ** Prerendering **是一种替代技术，如果它适合您的用例，更容易实现。这种方法不适用于高动态数据。 [prerender-spa-plugin]（https://www.npmjs.com/package/prerender-spa-plugin）允许您使用webpack实现它。
...
* Loading JavaScript *章节介绍了使用Babel和webpack的基本要点。不过，你应该执行React特有的设置。鉴于大多数React项目依赖于[JSX]（https://facebook.github.io/jsx/）格式，您必须通过Babel启用它。
...
接下来，React代码需要一个小入口点。如果您在浏览器端，则应将“Hello world``div`挂载到文档中。要证明它有效，单击它应该给出一个带有“hello”消息的对话框。在服务器端，返回React组件，服务器可以接收它。
...
您仍然缺少webpack配置来将此文件转换为服务器可以接收的内容。
...
为了保持良好，我们将定义一个单独的配置文件。已经完成了很多工作。鉴于您必须使用来自多个环境的相同输出，使用UMD作为库目标是有意义的：
...
为了清楚地理解，您可以设置一个独立的Express服务器来获取生成的捆绑包并按照SSR原则呈现它。首先安装Express：
...
即使现在运行React应用程序，也很难开发。如果您尝试修改代码，则不会发生任何事情。如本书前面所述，在多编译器模式下运行webpack可以解决该问题。另一种选择是针对当前配置以**监视模式**运行webpack，并为服务器设置监视器。您将接下来学习设置。
...
T>如果您按照* Separating a Manifest *章节中的说明编写清单，则可以将webpack生成的资产的引用自动写入服务器端模板。
...
在另一个终端中运行`node_modules / .bin / browser-refresh。/ server.js`并在之前打开浏览器`http：// localhost：8080`来测试设置。记得让webpack在另一个终端的监视模式下运行（`npm run build：ssr  -  --watch`）。如果一切正常，您对演示客户端脚本（* app / ssr.js *）所做的任何更改都应显示在浏览器中或导致服务器出现故障。
...
如果服务器崩溃，则会丢失WebSocket连接。在这种情况下，您必须在浏览器中强制刷新。如果服务器也是通过webpack管理的，那么问题本来就可以避免。
...
而不是“div”在哪里安装应用程序，您可以在那里看到所有相关的HTML。在这种特殊情况下并不多，但它足以展示这种方法。
...
*如何通过Node之外的其他东西运行服务器？一种选择是将Node实例包装在服务中，然后通过主机环境运行。理想情况下，结果将被缓存，您可以为每个平台找到更具体的解决方案。
...
* **服务器端渲染**可以为浏览器提供更多初始渲染。您可以立即显示标记，而不是等待JavaScript加载。
...
*服务器端呈现还允许您将初始数据有效负载传递到客户端，以避免对服务器进行不必要的查询。
...
*服务器端渲染不是没有成本的，并且它会导致新的问题，因为您需要更好的方法来处理方面，例如样式或路由。服务器和客户端环境的基本方式不同，因此必须编写代码，以使其不依赖于特定于平台的功能。
