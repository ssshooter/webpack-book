...
即使webpack经常用于捆绑单页面应用程序，也可以将它与多个单独的页面一起使用。这个想法类似于在* Targets *章节中生成多个输出文件的方式。但是，这一次，您必须生成单独的页面。这可以通过`HtmlWebpackPlugin`和一些配置来实现。
...
使用webpack生成多个页面时，您有以下几种可能性：
When generating multiple pages with webpack, you have a couple of possibilities:

*通过*多编译器模式*并返回一系列配置。只要页面是分开的，并且跨越它们共享代码的最小需求，该方法就可以工作。这种方法的好处是您可以通过[parallel-webpack]（https://www.npmjs.com/package/parallel-webpack）处理它，以提高构建性能。
...
*设置单个配置并提取共性。您执行此操作的方式可能会有所不同，具体取决于您的方式。
...
*如果您遵循[渐进式网络应用程序]（https://developers.google.com/web/progressive-web-apps/）（PWA）的想法，您最终可以使用** app shell **或a **页面shell **并在使用时加载应用程序的部分。
...
在实践中，您有更多的维度。例如，您必须为页面生成i18n变体。这些想法在基本方法之上发展。
...
要生成多个单独的页面，应以某种方式初始化它们。您还应该能够返回每个页面的配置，因此webpack会选择它们并通过多编译器模式处理它们。
...
问题是，如何为每个页面注入不同的脚本。在当前配置中，两者共享相同的`entry`。要解决此问题，您应该将`entry`配置移到较低级别并按页面进行管理。要使用脚本进行测试，请设置另一个入口点：
...
如果你构建应用程序（`npm run build`），你应该找到* another / index.html *。根据生成的代码，您可以进行以下观察：
...
在更复杂的应用程序中，您应该跨页面应用* Bundle Splitting *章节中介绍的技术。因此，删除多编译器模式是值得的。
...
需要进行调整以在页面之间共享代码。大多数代码可以保持不变。将它暴露给webpack的方式必须改变，以便它接收单个配置对象。由于`HtmlWebpackPlugin`默认选择所有块，您必须调整它以仅选取与每个页面相关的块：
...
如果你生成一个构建（`npm run build`），你应该注意到与你做的第一个多页构建相比有些不同。您只能找到一个，而不是两个清单文件。如果检查它，您会注意到它包含对生成的所有文件的引用。
...
详细研究条目特定文件可以发现更多信息。您可以看到它们指向清单的不同部分。清单根据条目运行不同的代码。不需要多个单独的清单。
...
如果通过将其与代码拆分和智能路由相结合来进一步推动这一想法，您将最终实现渐进式Web应用程序（PWA）的理念。 [webpack-pwa]（https://github.com/webpack/webpack-pwa）示例说明了如何通过app shell或页面shell使用webpack实现该方法。
...
App shell最初加载，它管理整个应用程序，包括其路由。页面外壳更精细，并且在使用应用程序时会加载更多内容。在这种情况下，应用程序的总大小更大。相反，您可以更快地加载初始内容。
...
Webpack允许您管理多个页面设置。 PWA方法允许应用程序在使用时加载，webpack允许实现它。
...
*多页面设置可以导致**渐进式Web应用程序**。在这种情况下，您可以使用各种webpack技术来提供快速加载并根据需要获取功能的应用程序。这种技术的两种风格都有其自身的优点。
