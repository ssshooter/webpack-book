...
由于其配置驱动的方法，它不是最容易学习的工具，但它非常强大。本指南的目的是帮助您开始使用webpack，然后超越基础知识。
...
Webpack并不是唯一可用的捆绑器，并且已经出现了一系列不同的工具。任务运行者，如Grunt和Gulp，是高级工具的很好的例子。通常问题是您需要手动编写工作流程。将该问题推向捆绑商（例如webpack）是向前迈出的一步。
...
Webpack采取另一种方式。它允许您将项目视为依赖图。您可以在项目中使用* index.js *，通过标准的`require`或`import`语句引入项目所需的依赖项。如果需要，您可以以相同的方式引用样式文件和其他资源。
...
Webpack为您完成所有预处理，并为您提供通过配置和代码指定的软件包。这种声明式方法是通用的，但很难学习。
...
在您开始了解Webpack的工作原理后，Webpack将成为不可或缺的工具。本书旨在通过最初的学习曲线，甚至更进一步。
...
本书教您开发用于开发和生产目的的可组合webpack配置。本书涵盖的高级技术使您可以充分利用webpack 4。
...
本书首先解释了webpack是什么。之后，您将找到多个章节，从不同的角度讨论webpack。在阅读这些章节时，您将开发自己的webpack配置，同时学习基本技术。
...
* **造型**非常注重造型相关主题。您将学习如何使用webpack加载样式以及如何在设置中引入自动修复等技术。
...
* ** Loading **详细解释了webpack的加载程序定义，并向您展示了如何加载图像，字体和JavaScript等资源。
...
* **输出**讨论了webpack的输出相关技术。尽管它的名字，它不仅适用于网络。您将了解如何使用webpack管理多个页面设置，并了解服务器端呈现的基本概念。
...
最后，有一个简短的结论章节回顾了本书的要点。它包含本书中的技术清单，允许您有条不紊地完成项目。
...
本书末尾的附录涵盖了第二个主题，有时还会深入探讨主要内容。您可以根据自己的兴趣以任何顺序接近它们。
...
最后的*故障排除*附录介绍了当webpack给您一个错误时该怎么做。它涵盖了一个过程，因此您知道该怎么做以及如何调试该问题。如有疑问，请研究附录。如果您不确定术语及其含义，请参阅本书末尾的*词汇表*。
...
您应该具备JavaScript，Node和npm的基本知识。如果您对webpack有所了解，那就太棒了。通过阅读本书，您可以加深对这些工具的理解。
...
如果您对该主题知之甚少，请考虑仔细阅读早期部分。您可以扫描其余部分来挑选您觉得有价值的部分。如果您已经了解webpack，请浏览并选择您认为有价值的技术。
...
如果您已经熟悉了webpack，那么本书中仍有一些内容供您使用。浏览它，看看你是否可以采用新技术。特别是阅读本章末尾和本书最后一章的摘要。
...
鉴于本书因创新步伐而获得了相当多的维护和改进，因此有一个版本控制方案。每个新版本的发行说明都保存在[书籍博客]（https://survivejs.com/blog/）。您也可以使用GitHub * compare *工具来实现此目的。例：
...
该页面显示了在给定版本范围之间进入项目的各个提交。您还可以在书中看到已更改的行。
...
如果您向Stack Overflow发布问题，请使用** survivaljs **标记它们。你可以在Twitter上使用hashtag **＃survivaljs **获得相同的结果。
...
非常感谢[Christian Alfoni]（http://www.christianalfoni.com/）帮助我制作了本书的第一版。这就是整个SurviveJS努力的灵感来源。您现在看到的版本是完全重写的。
...
没有最初的“SurviveJS  -  Webpack and React”努力，本书是不可能的。任何为此做出贡献的人都值得我的谢意。您可以查看该书以获得更准确的归因。
