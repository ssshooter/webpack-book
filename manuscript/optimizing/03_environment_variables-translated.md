...
有时，你的代码的一部分应该仅在开发期间执行。或者你可以在构建中具有尚未准备好进行生产的实验性功能。控制**环境变量**变得有价值，因为你可以使用它们切换功能。
...
T> `webpack.EnvironmentPlugin（[“NODE_ENV”]）`是一个允许你引用环境变量的快捷方式。它使用下面的`DefinePlugin`，你可以通过传递`process.env.NODE_ENV`来达到同样的效果。
...
本章中讨论的技术可用于根据环境选择整个模块。如上所示，基于“DefinePlugin”的拆分允许你选择要使用的代码分支以及要丢弃的代码。这个想法可用于在模块级别实现分支。考虑下面的文件结构：
...
*`DefinePlugin`基于**自由变量**运行，并在webpack分析源代码时替换它们。你可以使用Babel插件获得类似的结果。
...
*除了这些插件之外，你还可以找到其他与优化相关的插件，它们允许你以多种方式控制构建结果。
...
为确保构建具有良好的缓存失效行为，你将学习在下一章中将哈希包含到生成的文件名中。这样，客户端会注意到资产是否已更改，并且可以获取更新的版本。
